...
def load_openai_model(path: str = './openai/model/', ignore_mask: bool = False, use_one_embedding_dropout: bool = False,
                      use_decoder_bias: bool = False, debug: bool = False, max_len: int = 512,
                      compute_logit: bool = True) -> keras.Model:
    ...
    model = create_model(embedding_dim=768, embedding_dropout=0.1, vocab_size=40478 + TextEncoder.SPECIAL_COUNT,
                         ...
                         use_decoder_bias=use_decoder_bias, compute_logit=compute_logit)
    if debug:
...
def create_model(embedding_dim: int = 768, embedding_dropout: float = 0.1,
                 ...
                 compute_logit: bool = True, debug: bool = False) -> keras.Model:
    ...
    return keras.Model(inputs=[tokens, segment_ids, pos_ids] + ([] if ignore_mask else [mask]),
                       outputs=[x] + ([logits] if compute_logit else []), name='Transformer')
