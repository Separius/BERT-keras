...
my_model = create_transformer(embedding_layer_norm=True, neg_inf=-10000.0,
                              vocab_size=30522 - TextEncoder.SPECIAL_COUNT, accurate_gelu=True,
                              ln_epsilon=1e-12, max_len=seq_len, use_one_embedding_dropout=True)
...
if K.backend() == 'tensorflow':
    ...
    weights = [np.zeros(w.get_value().shape) for w in my_model.weights]
